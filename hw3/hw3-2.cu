...
__global__ void phase_1(int* d_dist, int padded_num_vertex, int round){
    ...
    __shared__ int share_target[BLOCKING_FACTOR*BLOCKING_FACTOR];
    #pragma unroll 4
    for(int id = 0; id < THREAD_LOAD_SAVE_NUM; id++){
        share_target[in_block_i*BLOCKING_FACTOR + in_block_j + id] =  d_dist[(glb_block_i*BLOCKING_FACTOR+in_block_i) * padded_num_vertex + (glb_block_j * BLOCKING_FACTOR + in_block_j) + id];
    }
    __syncthreads();
    ...
    for(int k=0;k<BLOCKING_FACTOR;k++){
        ...
        for(int id = 0; id<THREAD_LOAD_SAVE_NUM; id++){
            ...
            share_target[in_block_i*BLOCKING_FACTOR + in_block_j + id] = min(
                share_target[in_block_i*BLOCKING_FACTOR + in_block_j + id],
                share_target[in_block_i*BLOCKING_FACTOR + k] + share_target[k*BLOCKING_FACTOR + in_block_j + id]);
        }
    ...
    #pragma unroll 4
    for(int id = 0; id < THREAD_LOAD_SAVE_NUM; id++){
        d_dist[(glb_block_i*BLOCKING_FACTOR+in_block_i) * padded_num_vertex + (glb_block_j * BLOCKING_FACTOR + in_block_j) + id] = share_target[in_block_i*BLOCKING_FACTOR + in_block_j + id];
    }
}
...
__global__ void phase_2_fuse(int* d_dist, int padded_num_vertex, int round){
    ...
    extern __shared__ int to_phase_2[BLOCKING_FACTOR*BLOCKING_FACTOR];

    #pragma unroll 4
    for(int id=0;id<THREAD_LOAD_SAVE_NUM;id++){
        from_same_row[in_block_i*BLOCKING_FACTOR + in_block_j+id] = d_dist[(glb_b_i_same_row*BLOCKING_FACTOR+in_block_i)*padded_num_vertex + (glb_b_j_same_row*BLOCKING_FACTOR + in_block_j) + id];
        from_same_col[in_block_i*BLOCKING_FACTOR + in_block_j+id] = d_dist[(glb_b_i_same_col*BLOCKING_FACTOR+in_block_i)*padded_num_vertex+(glb_b_j_same_col*BLOCKING_FACTOR+in_block_j) + id];
        to_phase_2[in_block_i*BLOCKING_FACTOR + in_block_j+id]   = d_dist[(round*BLOCKING_FACTOR+in_block_i)* padded_num_vertex + (round*BLOCKING_FACTOR+in_block_j) + id];
    }
    __syncthreads();
    ...
    for(int k=0;k<BLOCKING_FACTOR;k++){
        #pragma unroll 4
        for(int id=0;id<THREAD_LOAD_SAVE_NUM;id++){
            from_same_row[in_block_i*BLOCKING_FACTOR + in_block_j+id] = min(
                from_same_row[in_block_i*BLOCKING_FACTOR + in_block_j+id],
                to_phase_2[in_block_i*BLOCKING_FACTOR + k] + from_same_row[k*BLOCKING_FACTOR + in_block_j+id]
            );
            from_same_col[in_block_i*BLOCKING_FACTOR + in_block_j+id] = min(
                from_same_col[in_block_i*BLOCKING_FACTOR + in_block_j+id],
                from_same_col[in_block_i*BLOCKING_FACTOR + k] + to_phase_2[k*BLOCKING_FACTOR + in_block_j+id]
            );
        }
        __syncthreads();
    }
    for(int id=0;id < THREAD_LOAD_SAVE_NUM;id++){
        d_dist[(glb_b_i_same_row*BLOCKING_FACTOR + in_block_i) * padded_num_vertex + (glb_b_j_same_row*BLOCKING_FACTOR+in_block_j) + id] = from_same_row[in_block_i*BLOCKING_FACTOR + in_block_j+id];
        d_dist[(glb_b_i_same_col*BLOCKING_FACTOR + in_block_i) * padded_num_vertex + (glb_b_j_same_col*BLOCKING_FACTOR+in_block_j) + id] = from_same_col[in_block_i*BLOCKING_FACTOR + in_block_j+id];
    }
}
...
void blocked_floyd_warshell(){
    ...
    // each block is BLOCKING_FACTOR * BLOCKING_FACTOR
    ...
    // num_blocked_square_row is guaranteed to be an integer, for padded_num_vertex = COALESCED_FACTOR * k, which is a multiple of BLOCKING_FACTOR
    ...
    for(int round = 0; round< num_blocked_square_row; round++){
        ...
        phase_1 <<< phase_1_grid, basic_block, BLOCKING_FACTOR*BLOCKING_FACTOR*1>>> (d_dist, padded_num_vertex, round);
