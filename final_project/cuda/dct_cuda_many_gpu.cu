...
double*** dct_compression(double*** image){
    ...
    #pragma omp parallel
    {
        ...
        if(gpu_id==0){
            ...
            cudaMemcpy(channels[gpu_id], d_channels[gpu_id], height*width*sizeof(double), cudaMemcpyDeviceToHost);
        }else{
            dct_kernel<<<grid, block>>>(d_channels[gpu_id], height, width, n, m, gpu_id);
            dct_kernel<<<grid, block>>>(d_channels[gpu_id], height, width, n, m, gpu_id+1);
            cudaMemcpy(channels[gpu_id], d_channels[gpu_id], height*width*sizeof(double), cudaMemcpyDeviceToHost);
            cudaMemcpy(channels[gpu_id+1], d_channels[gpu_id+1], height*width*sizeof(double), cudaMemcpyDeviceToHost);
        }
    }
