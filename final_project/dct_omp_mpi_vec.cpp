...
void matrix_mul_vec(double A[8][8], double B[8][8], double C[8][8]) {
  double zero[2] = {0};
  double aa[2], bb[2], cc[2];
  __m128d sum, a_vec, b_vec;

  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++){
      sum = _mm_load_pd(&zero[0]);
      for (int k = 0; k < 8; k+=2) {
        aa[0] = A[i][k];
        aa[1] = A[i][k+1];
        bb[0] = B[k][j];
        bb[1] = B[k+1][j];
        a_vec = _mm_load_pd(&aa[0]);
        b_vec = _mm_load_pd(&bb[0]);
        sum = _mm_add_pd(sum, _mm_mul_pd(a_vec, b_vec));
      }
      _mm_store_pd(&cc[0], sum);
      C[i][j] = cc[0] + cc[1];
    }
  }
}
...
double*** dct_compression(double*** image, unsigned int from_i, unsigned int to_i){
    int m=4, n=3; // m: quantization level, n: store subblock size
    ...
    for(unsigned int i=from_i;i<to_i;i+=8){
        for(unsigned int j=0;j<width;j+=8){
            ...
            for(unsigned int c=0;c<components;c++){
                ...
                // apply dct_vec
                matrix_mul_vec(DCT_MATRIX, temp, temp_dct_1);
                matrix_mul_vec(temp_dct_1, DCT_MATRIX_T, temp_dct_2);
                ...
                // apply idct_vec
                matrix_mul_vec(DCT_MATRIX_T, temp_dct_2, temp_idct_1);
                matrix_mul_vec(temp_idct_1, DCT_MATRIX, temp_idct_2);
                // apply idct_ori
